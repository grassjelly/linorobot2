...
def generate_launch_description():
    ...
    description_launch_path = PathJoinSubstitution(
        [FindPackageShare('linorobot2_description'), 'launch', 'description_mm.launch.py']
    )

    arm_controller_launch_path = PathJoinSubstitution(
        [FindPackageShare('robot_arm_gazebo'), 'launch', 'load_controllers.launch.py']
    )

    return LaunchDescription([
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(arm_controller_launch_path)
        ),
